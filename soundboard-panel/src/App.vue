<template >
	<div class="select-none min-h-screen bg-gray-900 text-gray-100 font-sans flex flex-col">
		<header class="p-4 border-b border-gray-700 flex justify-between items-center">
			<h1 class="text-2xl font-bold">Soundboard</h1>
			<div class="flex gap-2">
				<button @click="openUploadModal"
					class="cursor-pointer bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded">Add
					Sound</button>
			</div>
		</header>
		...
		<main>
			<div class="px-6 pt-6 flex flex-col w-full gap-2" v-for="category in categories" :key="category">
				<h2 class="text-xl font-bold">{{ category }}</h2>
				<div class="flex flex-row flex-wrap gap-2.5">
					<div v-for="sound in sounds.filter(s => s.category === category)" :key="sound.id"
						@click="playSound(sound.id)" @contextmenu="e => rightClick(e, sound)"
		...
		<div
				...
				<div class="relative inline-block w-64"
					...
					<button
						...
						<div class="flex items-center gap-3">
							<img v-if="selectedChannel" :src="selectedChannel.guildIcon" alt="icon"
								class="h-10 w-10 rounded-full" />
							<div class="flex flex-col items-start">
								<span class="font-semibold text-base">{{ selectedChannel?.guildName || 'Use /soundboard first'
								}}</span>
								<span class="text-sm text-gray-300">{{ selectedChannel?.channelName || '' }}</span>
							</div>
						</div>
						<svg 
					...
					<!-- Dropdown menu -->
					<ul v-if="channelSelectorOpen"
						class="w-full  absolute z-20 mb-2 bg-gray-700 rounded-lg shadow-lg max-h-60 overflow-auto bottom-full">

						<li v-for="(channel, index) in soundboardChannels" :key="index"
							@click="selectedChannel = channel; channelSelectorOpen"
							class="cursor-pointer flex items-center gap-3 px-4 py-2 hover:bg-gray-600">
							<img :src="channel.guildIcon" alt="icon" class="h-8 w-8 rounded-full" />
							<div class="flex flex-col items-start">
								<span class="font-semibold text-sm">{{ channel.guildName }}</span>
								<span class="text-xs text-gray-300">{{ channel.channelName }}</span>
							</div>
						</li>
					</ul>
				</div>
			...
			<div class="flex-1 flex justify-center">
				<svg 
					@click="stopSound"
				class="cursor-pointer h-15 w-15 fill-gray-200 hover:fill-gray-300" viewBox="0 0 512 512">
					<g>
		...
		<div v-if="modalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center">
			<div class="bg-gray-900 p-6 rounded shadow-lg w-96">
				<h2 class="text-xl font-bold mb-4">{{ editingSound ? 'Edit Sound' : 'Add Sound' }}</h2>
				<div class="flex flex-col gap-3">
					<input v-model="modalForm.title" placeholder="Title"
						class="p-2 rounded bg-gray-800 text-gray-100 border border-gray-700" />
					<input v-model="modalForm.author" placeholder="Author"
						class="p-2 rounded bg-gray-800 text-gray-100 border border-gray-700" />

					<!-- Category dropdown with new category option -->
					<div class="flex flex-col gap-3">
						<select v-model="modalForm.category"
							class="flex-1 p-2 rounded bg-gray-800 text-gray-100 border border-gray-700">
							<option disabled value="">Select category</option>
							<option v-for="cat in categories" :key="cat" :value="cat">{{ cat }}</option>
							<option value="__new">+ New Category</option>
						</select>
						<input v-if="modalForm.category === '__new'" v-model="newCategory" placeholder="New category"
							class="flex-1 p-2 rounded bg-gray-800 text-gray-100 border border-gray-700" />
					</div>
					...
					<input type="file" @change="onFileChange" accept="audio/*"
						class="p-2 rounded bg-gray-800 text-gray-100 border border-gray-700" />
					<!-- <p v-if="modalForm.url" class="text-gray-400 text-sm truncate">Selected file: {{ modalForm.url }}
					</p> -->
				</div>

				<div class="mt-4 flex justify-end gap-2">
					<button @click="closeModal" class="bg-gray-600 hover:bg-gray-700 px-3 py-1 rounded">Cancel</button>
					<button @click="saveSound" class="bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded">{{ editingSound ?
						'Save' : 'Add' }}</button>
				</div>
			</div>
		</div>
		...
		<div v-if="deleteModalOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center">
			<div class="bg-gray-900 p-6 rounded shadow-lg w-80">
				...
				<p class="mb-4">Are you sure you want to delete "{{ soundToDelete?.title }}"?</p>
				<div class="flex justify-end gap-2">
					<button @click="deleteModalOpen = false"
						class="bg-gray-600 hover:bg-gray-700 px-3 py-1 rounded">Cancel</button>
					<button @click="deleteSound" class="bg-red-600 hover:bg-red-700 px-3 py-1 rounded">Delete</button>
				</div>
			</div>
		</div>
	</div>
	...
	<!-- Context Menu -->
	<div v-if="rightClickedSound" class="absolute bg-white shadow-lg rounded-md border border-gray-200 w-40 z-50"
		:style="{ left: contextMenuX + 'px', top: contextMenuY + 'px' }">
		<ul class="flex flex-col">
			<li>
				<button
					class="block text-left cursor-pointer w-full px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-gray-900">Play</button>
			</li>
			<li>
				<button @click="openEditModal(rightClickedSound)"
					class="block text-left cursor-pointer w-full px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-gray-900">Edit</button>
			</li>
			<li>
				<a :href="getSoundURL(rightClickedSound.filename)" target="_blank" rel="noopener noreferrer"
					class="block text-left cursor-pointer w-full px-4 py-2 text-gray-700 hover:bg-gray-100 hover:text-gray-900">Download</a>
			</li>
			<li>
				<button @click="confirmDelete(rightClickedSound)"
					class="block text-left cursor-pointer w-full px-4 py-2 text-red-600 hover:bg-red-100 hover:text-red-700">Delete</button>
			</li>
		</ul>
	</div>
</template>
...
import { disconnectChannel, MessageType, playSound, stopSound } from './clients/ws-client'
...
function openUploadModal() {
	...
	Object.assign(modalForm, { title: '', author: '', category: '' })
